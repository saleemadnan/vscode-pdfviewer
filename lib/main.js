...
(function () {
  ...
  window.addEventListener('load', async function () {
    ...
    PDFViewerApplication.initializedPromise.then(() => {
      ...
      PDFViewerApplication.open(config.path).then(async function () {
        ...
        console.error('PDF Viewer: failed to open document', err)
      })
    })
    ...
    window.addEventListener('message', function () {
      ...
      pdfjsLib.getDocument(loadOpts).promise.then(function (doc) {
        doc._pdfInfo.fingerprints = [config.path]
        PDFViewerApplication.load(doc)
        PDFViewerApplication.pdfViewer._resetView = oldResetView
      }).catch(function (err) {
        console.error('PDF Viewer: failed to reload document', err)
        PDFViewerApplication.pdfViewer._resetView = oldResetView
      })
    });
  }, { once: true });
  ...
  window.onerror = function (message, source, lineno, colno, error) {
    console.error('PDF Viewer Error:', { message, source, lineno, colno, error })
    const container = document.createElement('div')
    container.style.cssText = 'padding: 20px; font-family: sans-serif; color: #d32f2f;'
    container.innerText = 'An error occurred while loading the file. Please close and reopen it.'
